...
/////////////////////////////////////////////////////////////////////
contract SharePriceVote_1 {
//////////////////////////////////COMMITTEE KEYS/////////////////////////////
address Apollo = 0x06400992be45bc64a52b5c55d3df84596d6cb4a1;

/*contract whitelist data structure*/
struct SharePriVote_1 {
bool authorised;
address addr;  
}
/*contract whitelist*/
mapping (uint8 => SharePriVote_1) public SharePriVotePass;
...
uint8 PriceProposal; //change exchange rate if vote passes
}	
...
/*store parameters for changing exchange rate*/
struct ChangeRate {
uint256 Rate;
}
...
uint8 ChangeVoteRules, uint8 PriceProposal); 
...
function SharePriceVote_1(){
Core.ShareQuorum = 30;	
Core.YesShares = 60;
Core.YesVoters = 60;
Core.ActivePro = false;
Core.ActiveRatePro = false;
Core.ActiveRulesPro = false;
Core.ConfSharePrice = block.number + 13000;		
}	

/////////////////////////INITIALIZATION PROTOCOL////////////////////////
function InitSharePrice(uint8 accountid_spv1,address addr_spv1){ 

if(msg.sender != Apollo) throw;                //check who is calling the function  
if(block.number > Core.ConfSharePrice) throw; //check if the initialization window has closed     

SharePriVotePass[accountid_spv1].authorised = true;
SharePriVotePass[accountid_spv1].addr = addr_spv1;
}///////////////////////////
...
SokMintOne
SokMintOneCall = SokMintOne(SharePriVotePass[5].addr);
...
uint8 PriceProposal) returns (bool sucess){
/*set up contract call*/
SokMintOne
SokMintOneCall = SokMintOne(SharePriVotePass[5].addr);
...
if (Core.ActiveRatePro == false && PriceProposal ==1) throw;
...
/*set up contract call*/
SokTreasuryOne
SokTreasuryOneCall = SokTreasuryOne(SharePriVotePass[8].addr);
Prop.IssuedFlorin = (SokTreasuryOneCall.IssuedShares());
Prop.ChangeVoteRules = ChangeVoteRules;	
Prop.PriceProposal = PriceProposal;	
Core.ActivePro = true;	
/*proposal event*/
ProposalLog (ProposalID, ProposalTitle,DocUrl,DocHash,NewtonValue,
Prop.StartingBlock,Prop.EndingBlock, ChangeVoteRules, PriceProposal); 
return true;	
}//////////////////
...
/*function for inputting details of price proposal*/
function NewRateProposal(uint256 NewRate)	
returns (bool sucess){	
/*set up call to sok mint one*/
SokMintOne
SokMintOneCall = SokMintOne(SharePriVotePass[5].addr);
if((SokMintOneCall.SirBal(msg.sender)) < 2) throw;  
if (Core.ActivePro == true) throw;
ChangeR.Rate = NewRate;
Core.ActiveRatePro = true;
return true;
}///////////////////////////////////////////
...
SokVoteRegister
SokVoteRegisterCall = SokVoteRegister(SharePriVotePass[10].addr);
...
if (YesResult() == true && Prop.PriceProposal == 1){
/*set up call to sok share one*/
SokShareOne
SokShareOneCall = SokShareOne(SharePriVotePass[11].addr);
(SokShareOneCall.UpdatePrice(ChangeR.Rate));
}  
if (YesResult() == true){
...
}////////////////////////////////////end of share_price_vote_1 contract
